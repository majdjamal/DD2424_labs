...
class ConvNet:
    ...
    def fit(self):
        ...
        ##  F1, F2, W, MF1, and MF2
        ##
        self.F1 = np.arange(n1*d*k1) + 1
        self.F1 = self.F1.reshape((n1, d, k1))

        self.F2 = np.arange(n2*n1*k2) + 1
        self.F2 = self.F2.reshape((n2, n1, k2))

        self.W = np.arange(Nout*n2*nlen2) + 1
        self.W = self.W.reshape((Nout, (n2 * nlen2)))
        ...
        s1 = self.MF1 @ X_train[:, 0].reshape(-1,1)
        s1 = ReLU(s1)
        s2 = self.MF2 @ s1
        o = self.W @ s2
        ...
        #s1_func,s2_func,p_func = self.forward(X_train[:, 0].reshape(-1,1), self.MF1, self.MF2, self.W)
        ...
        mx = self.MakeMXMatrix(s1, *self.F2.shape, n2, nlen1)
        print(mx.shape)
        print(g.shape)
        v = g.T @ mx
        ...
        mx = self.MakeMXMatrix(x, *self.F1.shape, d, nlen)
